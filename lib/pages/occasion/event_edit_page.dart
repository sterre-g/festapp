...
class _EventEditPageState extends State<EventEditPage> {
  ...
  void validateForm() {
    setState(() {
      isFormValid = title != null && title!.trim().isNotEmpty && startDate != null && endDate != null && !endDate!.isBefore(startDate!);
    });
  }
    ...
    if (isFormValid && _formKey.currentState!.validate()) {
      ...
      if (originalEvent != null) {
        originalEvent!
          ...
          ..type = type
          ..description = content
          ..parentEventIds = showInsideEvent != null && showInsideEvent!.isNotEmpty ? showInsideEvent!.split(",").map((e) => int.parse(e.trim())).toList() : [];
                            ...
                            TextFormField(
                              ...
                              decoration: InputDecoration(
                                ...
                                labelStyle: TextStyle(
                                  color: (title == null || title!.trim().isEmpty) ? ThemeConfig.redColor(context) : null,
                                ),
                              ),
                              ...
                              child: ElevatedButton(
                                onPressed: () async {
                                  RouterService.navigatePageInfo(
                                    context,
                                    HtmlEditorRoute(content: {HtmlEditorPage.parContent: content}, occasionId: originalEvent!.occasionId),
                                  ).then((value) {
                                    if (value != null) {
                                      setState(() {
                ...
                ElevatedButton(
                  ...
                  style: ElevatedButton.styleFrom(
                    ...
                    backgroundColor: isFormValid
                        ...
                        : ThemeConfig.appBarColor().withValues(alpha: 0.5), // Semi-transparent appBarColor for disabled state
                    foregroundColor: isFormValid
                        ? null // Default text color for enabled state
                        : ThemeConfig.grey600(context), // Subtle gray text for disabled state
                    disabledBackgroundColor: ThemeConfig.appBarColor().withValues(alpha: 0.5), // Ensure visible disabled background
