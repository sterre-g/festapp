    ...
    controller ??= SingleDataGridController<PlaceModel>(
      ...
      columns: [
        ...
        TrinaColumn(
          ...
          width: 100,
          renderer: (ctx) => DataGridHelper.checkBoxRenderer(ctx, Tb.places.is_hidden),
        ),
        TrinaColumn(
          title: "Title".tr(),
        ...
        TrinaColumn(
          ...
          renderer: (ctx) {
            return DataGridHelper.buildHtmlEditorButton(
              ...
              field: Tb.places.description,
              rendererContext: ctx,
              loadContent: () async => ctx.row.cells[Tb.places.description]!.value,
            );
          },
        ),
        TrinaColumn(
          ...
          applyFormatterInEditing: true,
          formatter: (d) => svgIcons.firstWhereOrNull((i) => i.id == d)?.link ?? PlaceModel.WithouValue,
          type: TrinaColumnType.select(
            defaultValue: null,
            mapIcons,
            builder: (icon) => DataGridHelper.iconToRow(context, icon, svgIcons),
          ),
          renderer: (ctx) => DataGridHelper.mapIconRenderer(context, ctx, svgIcons),
        ),
        TrinaColumn(
          ...
          renderer: (ctx) {
            return ElevatedButton(
              onPressed: () async {
                ...
                var value = await RouterService.navigatePageInfo(context, MapRoute(place: pm));
                if (value != null) {
                  var cell = ctx.row.cells[Tb.places.coordinates]!;
                  ctx.stateManager.changeCellValue(cell, value.toString(), force: true);
