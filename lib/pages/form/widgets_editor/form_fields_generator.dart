...
class _FormFieldsGeneratorState extends State<FormFieldsGenerator> {
  ...
  void initState() {
    super.initState();
    final topLevelFields = widget.form.relatedFields?.where((f) => f.isTicketField != true).toList() ?? [];
    ...
    return ReorderableListView.builder(
      ...
      onReorder: (oldIndex, newIndex) {
        setState(() {
          ...
          updatedList.addAll(widget.form.relatedFields!.where((f) => f.isTicketField == true));
          ...
          } else if (selectedIndex != null && selectedIndex! >= newIndex && selectedIndex! < oldIndex) {
  ...
  Widget _buildFieldItem(BuildContext context, int index, FormFieldModel field, List<FormFieldModel> displayList) {
      ...
      child: Opacity(
        ...
        child: Card(
          ...
          child: Padding(
            ...
            child: isSelected ? _buildFieldItemSelected(field, displayList, index) : _buildFieldItemNonSelected(field),
          ),
        ),
      ),
    );
  ...
  Widget _buildFieldItemNonSelected(FormFieldModel field) {
    ...
    final displayTitle = (field.title?.isNotEmpty ?? false) ? field.title! : FormHelper.fieldTypeToLocale(field.type!);
    ...
    return Column(
      ...
      children: [
        ...
        if (isTicket)
          Row(
            children: [
              Icon(FormHelper.fieldTypeIcons[FormHelper.fieldTypeTicket], size: 24, color: Theme.of(context).colorScheme.primary),
              const SizedBox(width: 8),
          ...
          children: [
            ...
            Row(
              children: [
                ...
                Checkbox(
                  value: FormHelper.isAlwaysRequired(field.type ?? '') || (field.isRequired ?? false),
                  onChanged: FormHelper.isAlwaysRequired(field.type ?? '') ? null : (value) => setState(() => field.isRequired = value),
                ),
              ],
            ),
            const SizedBox(width: 16),
            Row(
              children: [
                ...
                Switch(
                  ...
                  onChanged: (field.type == FormHelper.fieldTypeEmail || isTicket) ? null : (value) => setState(() => field.isHidden = !value),
                ),
              ],
            ),
            if (!isTicket)
  ...
  Widget _buildAnswerWidget(BuildContext context, FormFieldModel field, bool isEditable) {
    switch (field.type) {
      case FormHelper.fieldTypeTicket:
        return isEditable
            ? TicketEditorWidgets.buildTicketEditor(context, widget.form, field, () {
                setState(() {});
              })
            : TicketEditorWidgets.buildTicketEditorReadOnly(context, widget.form, field);
      case FormHelper.fieldTypeSelectOne:
        return isEditable ? SelectOneEditor.buildSelectOneEditor(context, field, widget.form.occasion) : SelectOneEditor.buildSelectOneReadOnly(context, field);
      case FormHelper.fieldTypeSelectMany:
        return isEditable ? SelectManyEditor.buildSelectManyEditor(context, field, widget.form.occasion) : SelectManyEditor.buildSelectManyReadOnly(context, field);
      case FormHelper.fieldTypeSex:
        return SexEditor.buildSexFieldReadOnly(context, field);
      case FormHelper.fieldTypeBirthDate:
        return isEditable ? BirthDateEditor.buildBirthDateEditor(context, field, widget.form.occasion) : BirthDateEditor.buildBirthDateReadOnly(context, field);
      default:
        if (isEditable) {
          return Padding(
  ...
  List<String> get _availableFieldTypes {
    final existingTypes = widget.form.relatedFields?.map((f) => f.type).toList() ?? [];
