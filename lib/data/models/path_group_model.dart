...
class PathGroupsModel extends ITrinaRowModel {
  ...
  factory PathGroupsModel.fromJson(Map<String, dynamic> json) {
    ...
    final pathData = raw?.cast<List<dynamic>>().map((sub) => sub.cast<int>().toList()).toList();
  ...
  static PathGroupsModel fromPlutoJson(Map<String, dynamic> json) {
    ...
    final parsedData = rawData.isNotEmpty ? jsonDecode(rawData) as Map<String, dynamic> : <String, dynamic>{};
    ...
    final hidden = rawHidden is bool ? rawHidden : (rawHidden?.toString().toLowerCase() == 'true');
    ...
    return PathGroupsModel(
      id: json[Tb.path_groups.id] == -1 ? null : json[Tb.path_groups.id] as int?,
      title: json[Tb.path_groups.title] as String?,
      data: parsedData,
      pathData: pathData,
  ...
  TrinaRow toTrinaRow(BuildContext context) {
    ...
    final placesCsv = (pathData != null && pathData!.isNotEmpty) ? pathData!.map((sub) => sub.map((i) => i.toString()).join(',')).join(';') : "";
