...
import 'package:fstapp/data/models/icon_model.dart';
...
class DbPlaces {
  ...
  static Future<List<PlaceModel>> getMapPlaces() async {
    var data = await _supabase.from(Tb.places.table).select().eq(Tb.places.is_hidden, false).eq(Tb.places.occasion, RightsService.currentOccasionId!);
  ...
  static Future<List<PlaceModel>> getAllPlaces() async {
    var data = await _supabase.from(Tb.places.table).select().eq(Tb.places.occasion, RightsService.currentOccasionId!);
    var toReturn = List<PlaceModel>.from(data.map((x) => PlaceModel.fromJson(x)));
    toReturn.sortPlaces();
    return toReturn;
  }
  ...
  static Future<List<PlaceModel>> getPlacesIn(List<int> ids) async {
    var data = await _supabase.from(Tb.places.table).select().inFilter(Tb.places.id, ids);
    var toReturn = List<PlaceModel>.from(data.map((x) => PlaceModel.fromJson(x)));
    toReturn.sortPlaces();
    return toReturn;
  }
  ...
  static Future<List<PathGroupsModel>> getAllPathGroups() async {
    ...
    var toReturn = (data as List<dynamic>).map((x) => PathGroupsModel.fromJson(x as Map<String, dynamic>)).toList();
  ...
  static Future<void> updatePathGroup(PathGroupsModel model) async {
    ...
    if (model.id != null) {
      ...
      pgData = await _supabase.from(Tb.path_groups.table).update(upsertObj).eq(Tb.path_groups.id, model.id!).select().single();
    } else {
      // --- INSERT new
      upsertObj[Tb.path_groups.occasion] = RightsService.currentOccasionId!;
      pgData = await _supabase.from(Tb.path_groups.table).insert(upsertObj).select().single();
    }
  ...
  static Future<void> deletePathGroup(PathGroupsModel model) async {
    await _supabase.from(Tb.path_groups.table).delete().eq(Tb.path_groups.id, model.id!);
  }
}
