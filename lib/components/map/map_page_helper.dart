...
class MapPageHelper {
  ...
  static Future<Map<int, List<fm.Polyline>>> loadGroupPolylines(
    ...
    for (var g in groups) {
      ...
      final pathColor = g.color != null ? Color(int.parse(g.color!.replaceFirst('#', '0x'))) : Colors.blue;
      final lines = <fm.Polyline>[];

      for (var segment in g.pathData!) {
        // map segment (List<int>) -> List<LatLng>
        final pts = List<LatLng>.from(
          segment.map((id) => placesList.firstWhereOrNull((pl) => pl.id == id)).whereType<PlaceModel>().map((p) => LatLng(p.getLat(), p.getLng())),
        );
      ...
      return _buildIconColumn(
        ...
        (id) {
          ...
          if (isMobile && scrollController != null) {
            ...
            final target = i * (itemWidth + itemSpacing) - (screenW - itemWidth) / 2;
  ...
  static Widget _buildIconColumn(
    ...
    Widget badge = (iconSvg != null) ? SvgPicture.string(iconSvg, width: 24, height: 24) : Icon(Icons.route_outlined, size: 24, color: pathColor);
  ...
  static Widget buildSelectedGroupTitle(
    ...
    final pathColor = selectedGroup.color != null ? Color(int.parse(selectedGroup.color!.replaceFirst('#', '0x'))) : Colors.blue;
